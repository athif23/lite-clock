...
public:
	...
	*/
	...
	PrayerTimes(CalculationMethod calc_method = Egypt,
		JuristicMethod asr_juristic = Shafii,
		AdjustingMethod adjust_high_lats = AngleBased,
		double dhuhr_minutes = 0)
	...
	void _get_prayer_times(int year, int month, int day, double _latitude, double _longitude, double __timezone, double times[])
	{
		latitude = _latitude;
	...
	void get_prayer_times(time_t date, double latitude, double longitude, double timezone, double times[])
	{
		...
		_get_prayer_times(1900 + t.tm_year, t.tm_mon + 1, t.tm_mday, latitude, longitude, timezone, times);
	}
	...
	{
		tm tmp;
		localtime_s(&tmp, &local_time);
		tmp.tm_isdst = 0;
		time_t local = mktime(&tmp);
		gmtime_s(&tmp, &local_time);
		tmp.tm_isdst = 0;
